...
test.describe("User Story 2: Enter Annual Consumption", () => {
  test('Verify Visibility & Initial Input Value "Jahresverbrauch" field', async ({
    page,
  }) => {
  ...
  test("Verify Invalid Consumption Data", async ({ page }) => {
    ...
    const invalidValues = [
      {
        value: "-100",
        expectedError: "Bitte geben Sie Ihren Jahresverbrauch ein",
      }, // Negative value
      {
        value: "1000000",
        expectedError:
          "Bitte geben Sie einen Jahresverbrauch bis maximal 40,000 kWh ein.",
      }, // Exceeding range
    ];
  ...
  test("Re-enter Correct Value After Error", async ({ page }) => {
    ...
    // Verify that the error message is visible and correct
    await expect(errorMessageContainer).toBeVisible();
    await expect(errorMessageContainer).toHaveText(
      "Bitte geben Sie Ihren Jahresverbrauch ein"
    );
